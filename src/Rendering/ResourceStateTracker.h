...
class CResourceStateTracker
{
public:
    CResourceStateTracker();
    virtual ~CResourceStateTracker();
	 ...
	 */
	uint32_t FlushPendingResourceBarriers(CCmdList& commandList);

	/**
	 * Flush any (non-pending) resource barriers that have been pushed to the resource state
	 * tracker.
	 */
	void FlushResourceBarriers(CCmdList& commandList);
...
private:
	...
	TArray<D3D12_RESOURCE_BARRIER> PendingResourceBarriers;
	...
	using ResourceStateMap = std::map<ID3D12Resource*, ResourceState>;
