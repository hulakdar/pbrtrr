...
float2 packSnorm8Float(float2 v) {
    return float2(packSnorm8Float(v.x), packSnorm8Float(v.y));
}
...
float2 packSnormFloor8Float(float2 v) {
    return float2( packSnormFloor8Float(v.x), packSnormFloor8Float(v.y) );
}
...
float2 unpackSnorm8(float2 v) {
    return float2(unpackSnorm8(v.x), unpackSnorm8(v.y));
}
...
uint packVec2Into2Unorm8s(float2 v) {
    return pack2Norm8s(packUnorm8(v.x), packUnorm8(v.y));
}

uint packVec2Into2Snorm8s(float2 v) {
    return pack2Norm8s(packSnorm8(v.x), packSnorm8(v.y));
}

float2 unpack2Unorm8sIntoVec2(uint u) {
    uint x_int, y_int;
    unpack2Norm8s(u, x_int, y_int);
    return float2(unpackUnorm8(x_int), unpackUnorm8(y_int));
}

float2 unpack2Snorm8sIntoVec2(uint u) {
    uint x_int, y_int;
    unpack2Norm8s(u, x_int, y_int);
    return float2(unpackSnorm8(x_int), unpackSnorm8(y_int));
}
...
float2 packSnormFloor12Float(float2 v) {
    return float2( packSnormFloor12Float(v.x), packSnormFloor12Float(v.y) );
}
...
float2 unpackSnorm12(float2 v) {
    return float2(unpackSnorm12(v.x), unpackSnorm12(v.y));
}
...
uint packVec2Into2Unorm12s(float2 v) {
    return pack2Norm12s(packUnorm12(v.x), packUnorm12(v.y));
}

uint packVec2Into2Snorm12s(float2 v) {
    return pack2Norm12s(packSnorm12(v.x), packSnorm12(v.y));
}

float2 unpack2Unorm12sIntoVec2(uint u) {
    uint x_int, y_int;
    unpack2Norm12s(u, x_int, y_int);
    return float2(unpackUnorm12(x_int), unpackUnorm12(y_int));
}

float2 unpack2Snorm12sIntoVec2(uint u) {
    uint x_int, y_int;
    unpack2Norm12s(u, x_int, y_int);
    return float2(unpackSnorm12(x_int), unpackSnorm12(y_int));
}
...
float2 packSnorm16Float(float2 v) {
    return float2(packSnorm16Float(v.x), packSnorm16Float(v.y));
}
...
float2 packSnormFloor16Float(float2 v) {
    return float2( packSnormFloor16Float(v.x), packSnormFloor16Float(v.y) );
}
...
float2 unpackSnorm16(float2 v) {
    return float2(unpackSnorm16(v.x), unpackSnorm16(v.y));
}
...
uint packVec2Into2Unorm16s(float2 v) {
    return pack2Norm16s(packUnorm16(v.x), packUnorm16(v.y));
}

uint packVec2Into2Snorm16s(float2 v) {
    return pack2Norm16s(packSnorm16(v.x), packSnorm16(v.y));
}

float2 unpack2Unorm16sIntoVec2(uint u) {
    uint x_int, y_int;
    unpack2Norm16s(u, x_int, y_int);
    return float2(unpackUnorm16(x_int), unpackUnorm16(y_int));
}

float2 unpack2Snorm16sIntoVec2(uint u) {
    uint x_int, y_int;
    unpack2Norm16s(u, x_int, y_int);
    return float2(unpackSnorm16(x_int), unpackSnorm16(y_int));
}


#define LOWER_8_MASK 255

int3 packUInt24IntoUVec3(uint u) {
    return int3((u >> 16),
                 (u >> 8) & LOWER_8_MASK,
                 u & LOWER_8_MASK);
}

uint unpackUVec3IntoUInt(int3 u) {
    return u.z + (u.y << uint(8)) + (u.x << uint(16));
}

int3 vec2To2Unorm12sEncodedAsUVec3(float2 v) {
    return packUInt24IntoUVec3(packVec2Into2Unorm12s(v));
}

int3 vec2To2Snorm12sEncodedAsUVec3(float2 v) {
    return packUInt24IntoUVec3(packVec2Into2Snorm12s(v));
}

float2 twoNorm12sEncodedAsUVec3InVec3FormatToPackedVec2(float3 v) {
    float2 s;
...
float2 twoUnorm12sEncodedAsUVec3InVec3FormatToVec2(float3 v) {
    float2 s = twoNorm12sEncodedAsUVec3InVec3FormatToPackedVec2(v);
    return float2(unpackUnorm12(s.x), unpackUnorm12(s.y));
}

float2 twoSnorm12sEncodedAsUVec3InVec3FormatToVec2(float3 v) {
    float2 s = twoNorm12sEncodedAsUVec3InVec3FormatToPackedVec2(v);
    return float2(unpackSnorm12(s.x), unpackSnorm12(s.y));
}

float2 twoNorm12sEncodedAsUVec3ToPackedVec2(int3 u) {
    float2 s;
    float3 u_f = float3(u);
...
float2 twoUnorm12sEncodedAsUVec3ToVec2(int3 u) {
    float2 s = twoNorm12sEncodedAsUVec3ToPackedVec2(u);
    return float2(unpackUnorm12(s.x), unpackUnorm12(s.y));
}

float2 twoSnorm12sEncodedAsUVec3ToVec2(int3 u) {
    float2 s = twoNorm12sEncodedAsUVec3ToPackedVec2(u);
    return float2(unpackSnorm12(s.x), unpackSnorm12(s.y));
}

float3 twoNorm12sEncodedAs3Unorm8sInVec3Format(float2 s) {
    float3 u;
    u.x = s.x * (1.0/16.0);
    float t = floor(s.y*(1.0/256));
    u.y = (frac(u.x)*256) + t;
    u.z = s.y - (t * 256);
    // Instead of a floor, you could just add float3(-0.5) to u, 
    // and the hardware will take care of the flooring for you on save to an RGB8 texture
    return floor(u) * (1.0 / 255.0);
}

float3 vec2To2Snorm12sEncodedAs3Unorm8sInVec3Format(float2 v) {
    float2 s = float2(packSnorm12Float(v.x), packSnorm12Float(v.y));
    return twoNorm12sEncodedAs3Unorm8sInVec3Format(s);
}

float3 vec2To2Unorm12sEncodedAs3Unorm8sInVec3Format(float2 v) {
    float2 s = float2(packUnorm12Float(v.x), packUnorm12Float(v.y));
    return twoNorm12sEncodedAs3Unorm8sInVec3Format(s);
}
